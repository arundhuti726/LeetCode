...
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    # Merge orders and company DataFrames
    d = orders.merge(company, left_on='com_id', right_on='com_id', how='left')

    # Update sales_id based on the condition
    d['sales_id'] = np.where(d['name'] == 'RED', d['sales_id'], np.nan)

    # Merge sales_person with the updated DataFrame
    merged_df = sales_person.merge(d[['sales_id']], on='sales_id', how='left', indicator=True)
